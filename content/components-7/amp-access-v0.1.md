...
The [`amp-subscriptions`](https://github.com/ampproject/amphtml/blob/master/extensions/amp-access/../amp-subscriptions/amp-subscriptions.md) extension offers
...
_&lt;URL&gt;_ values specify HTTPS URLs with substitution variables. The substitution variables are covered in more detail in the [Access URL Variables][7] section below.
...
<section amp-access="views <= maxViews">
  <template amp-access-template type="amp-mustache">
    You are reading article {% raw %}{{views}}{% endraw %} out of {% raw %}{{maxViews}}{% endraw %}.
  </template>
</section>
[/sourcecode]
...
In the _server_ option, the call to the Authorization endpoint is made by the Google AMP Cache as a simple HTTPS endpoint. This means that the Publisher’s cookies cannot be delivered in this case.
...
The integration with _amp-analytics_ is documented in the [amp-access-analytics.md](https://github.com/ampproject/amphtml/blob/master/extensions/amp-access/./amp-access-analytics.md).
