...
`data-dismiss-href` URL. Be very mindful of the browser caching the `GET` response; see details below in the [`data-show-if-href`](https://github.com/ampproject/amphtml/blob/master/extensions/amp-user-notification/<#data-show-if-href-(optional)>) section. (We recommend
...
on the `data-show-if-href` attribute (see {{'[% raw %]'}}[#{{'{% endraw %}'}}1228](https://github.com/ampproject/amphtml/issues/1228) on why this is a GET instead of a POST).
